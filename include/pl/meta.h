...
// Evaluates to whether the argument's type is an integer type.
 ...
 #define pl_is_int(...) (__builtin_classify_type(pl_fake(__VA_ARGS__))==1||__builtin_classify_type(pl_fake(__VA_ARGS__))==18)
#else
...
// If the argument's type is not an integer type, evaluates to zero.
 ...
 #define pl_int_width(...) ((size_t)pl_choose(pl_is_int(__VA_ARGS__),__builtin_popcountg((unsigned _BitInt(BITINT_MAXWIDTH))__extension__ _Maxof(pl_choose_type(pl_is_int(__VA_ARGS__),typeof(__VA_ARGS__),0)))+((typeof(__VA_ARGS__))-1<0),0))
#else
