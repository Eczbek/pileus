...
#define pl_decay(/*type*/...) typeof(_Generic(typeof_unqual(__VA_ARGS__),unsigned char:pl_fake(__VA_ARGS__),unsigned short:pl_fake(__VA_ARGS__),signed char:pl_fake(__VA_ARGS__),short:pl_fake(__VA_ARGS__),char:pl_fake(__VA_ARGS__),bool:pl_fake(__VA_ARGS__),default:0?pl_fake(__VA_ARGS__):pl_fake(__VA_ARGS__)))
...
// If the argument's type is not an integer type, evaluates to zero.
 ...
 #define pl_int_width(/*int_type*/...) ((size_t)pl_choose(pl_is_int(__VA_ARGS__),__builtin_popcountg((unsigned _BitInt(BITINT_MAXWIDTH))__extension__ _Maxof(pl_choose_type(pl_is_int(__VA_ARGS__),typeof(__VA_ARGS__),0)))+((typeof(__VA_ARGS__))-1<0),0))
#else
